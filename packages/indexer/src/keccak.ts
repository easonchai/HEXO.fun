 ...
 * NIST SHA3-256 (`crypto.createHash("sha3-256")` uses 0x06 padding and would
 * produce different roots than the on-chain program).
 *
 * Why hand-rolled: `@noble/hashes` is present inside node_modules/.pnpm only as
 * a transitive artifact and is not resolvable from this workspace package
 * without running `pnpm install`, which is not permitted here. ~70 lines of
 * Keccak-f[1600] is cheaper than a dependency-graph change.
 */
...
export function keccak256(input: Uint8Array): Buffer {
  ...
  for (let offset = 0; offset < paddedLength; offset += RATE_BYTES) {
    ...
    for (let lane = 0; lane < 25; lane += 1) state[lane] ^= block[lane];
    keccakF1600(state);
  }
...
/** Dev-only oracle used by the keccak unit test to prove padding correctness. */
export function sha3_256Reference(input: Uint8Array): Buffer {
