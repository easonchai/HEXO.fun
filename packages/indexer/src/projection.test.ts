...
describe('HexVaultProjection', () => {
  it('is idempotent and records matching principal/entry minting', () => {
    ...
    const deposit = event('DepositRecorded', { owner: 'alice', amount: '1000000' }, 0);
  ...
  it('projects immutable positions and settlement with an in-range tile', () => {
    ...
    projection.apply(
      event(
        'PositionPurchased',
        { owner: 'alice', round: 'round-1', epochId: '3', roundId: '7', totalStake: '50' },
        0,
      ),
    );
    projection.apply(event('RoundSettled', { round: 'round-1', winningTile: '35' }, 1));
  ...
  it('rejects a late finalized event instead of silently corrupting a checkpoint', () => {
    ...
    expect(() => projection.apply(event('PrizeFunded', {}, 2))).toThrow('out-of-order');
  });
});
